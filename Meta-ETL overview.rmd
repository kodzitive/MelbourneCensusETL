...
The files were read (and joined when appropriate) via the statistics program R. Each output file was saved to it's own reproducable file named <City_year_diversitindices.csv> (eg. `Melbourne_2001_diversityindices.csv`)

### Diversity indices cleaned and rescaled

Each of `diversityindices.csv` files were read in, total rows were droped, rows with population below 10 had their values changed to NA, and non-tertile coulmns having a value below .01 were changed to NA. the remaining non-tertile values were then rescaled from 0 to 1 via (x-min(x))/range(x). Each resulting file was saved to a file named <City_year_diversityindices_cleanrescale.csv> (eg. `Melborune_2001_diversityindicies_cleanrescale.csv`).
